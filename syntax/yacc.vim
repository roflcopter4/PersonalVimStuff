...
if exists('g:yacc_uses_golang')
    let s:CodeType = '/go.vim'
elseif exists('g:yacc_uses_cpp')
    let s:CodeType = '/cpp.vim'
else
    let s:CodeType = '/c.vim'
endif

let s:Cpath= fnameescape(expand("<sfile>:p:h").s:CodeType)
if !filereadable(s:Cpath)
 for s:Cpath in split(globpath(&rtp, s:CodeType),"\n")
