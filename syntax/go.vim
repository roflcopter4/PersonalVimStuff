...
" Floating point
syn match       goFloat             "\<-\=\d\+\.\d*\%([Ee][-+]\=\d\+\)\=\>" contains=goExpChar
syn match       goFloat             "\<-\=\.\d\+\%([Ee][-+]\=\d\+\)\=\>" contains=goExpChar

" Imaginary literals
syn match       goImaginary         "\<-\=\d\+i\>"
syn match       goImaginary         "\<-\=\d\+[Ee][-+]\=\d\+i\>" contains=goExpChar
syn match       goImaginaryFloat    "\<-\=\d\+\.\d*\%([Ee][-+]\=\d\+\)\=i\>" contains=goExpChar
syn match       goImaginaryFloat    "\<-\=\.\d\+\%([Ee][-+]\=\d\+\)\=i\>" contains=goExpChar

syn match	goOctalPrefix	display contained "\%([^.]\)\@1<=\<0"
syn match	goHexPrefix	  display contained "\<0[xX]"

" Integers
syn match       goOctalError        "\%([^.]\)\@1<=\<-\=0\o*[89]\d*\>" contains=goOctalPrefix contained
syn match       goDecimalInt        "\<-\=\d\+\%([Ee][-+]\=\d\+\)\=\>" contains=goOctalError,goExpChar
syn match       goHexadecimalInt    "\<-\=0[xX]\x\+\>" contains=goHexPrefix
syn match       goOctalInt          "\<-\=0\o\+\>" contains=goOctalPrefix

syn match goExpChar "\%(\d\+\)\@22<=[eE]\%(\d\+\)\@=" contained 

hi def link     goHexPrefix         goSpecialNumber
hi def link     goOctalPrefix       goSpecialNumber
hi def link     goExpChar           goSpecialNumber
hi def link     goFloat             Float
hi def link     goImaginary         Number
hi def link     goImaginaryFloat    Float
hi def link     goDecimalInt        Integer
...
hi def link     goSpecialNumber     cNumberPrefix
