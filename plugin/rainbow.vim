...
func s:resolve_parenthesis(p)
    let l:ls = split(a:p, '\v%(%(start|step|end)\=(.)%(\1@!.)*\1[^ ]*|\w+%(\=[^ ]*)?) ?\zs', 0)
    let [l:paren, l:containedin, l:contains, l:op] = ['', '', 'TOP', '']
    for l:s in l:ls
        let [l:k, l:v] = [matchstr(l:s, '^[^=]\+\ze='), matchstr(l:s, '^[^=]\+=\zs.*')]
        if l:k ==# 'step'
            let l:op = l:v
        elseif l:k ==# 'contains'
            let l:contains = l:v
        elseif l:k ==# 'containedin'
            let l:containedin = l:v
        else
            let l:paren .= l:s
        endif
    endfor
    return [l:paren, l:containedin, l:contains, l:op]
endfunc
...
func rainbow#load()
    ...
    for l:parenthesis_args in l:conf.parentheses
        ...
        for l:lvl in range(l:maxlvl)
            ...
            if l:lvl == 0
                if l:containedin ==# ''
                    exe printf(l:def_rg, 'rainbow_r0', 'rainbow_p0', 'rainbow_r'.(l:maxlvl - 1), l:contains, l:paren)
                endif
            else
...
func rainbow#clear()
    ...
    if exists('b:rainbow_loaded')
        for l:each in range(b:rainbow_loaded)
            exe 'syn clear rainbow_r'.l:each
            exe 'syn clear rainbow_o'.l:each
        endfor
...
func rainbow#hide()
    if exists('b:rainbow_visible')
        for l:each in range(b:rainbow_loaded)
            exe 'hi clear rainbow_p'.l:each
            exe 'hi clear rainbow_o'.l:each
        endfor
